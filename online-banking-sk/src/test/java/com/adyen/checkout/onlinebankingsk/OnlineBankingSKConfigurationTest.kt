...
internal class OnlineBankingSKConfigurationTest {
    ...
    fun `when creating the configuration through CheckoutConfiguration, then it should be the same as when the builder is used`() {
        ...
        val actual = checkoutConfiguration.getOnlineBankingSKConfiguration()

        val expected = OnlineBankingSKConfiguration.Builder(
            shopperLocale = Locale.US,
            environment = Environment.TEST,
            clientKey = TEST_CLIENT_KEY,
        )
    ...
    fun `when the configuration is mapped to CheckoutConfiguration, then CheckoutConfiguration is created correctly`() {
        val config = OnlineBankingSKConfiguration.Builder(
            shopperLocale = Locale.US,
            environment = Environment.TEST,
            clientKey = TEST_CLIENT_KEY,
        )
        ...
        val actualSkConfig = actual.getOnlineBankingSKConfiguration()
        assertEquals(config.shopperLocale, actualSkConfig?.shopperLocale)
        assertEquals(config.environment, actualSkConfig?.environment)
        assertEquals(config.clientKey, actualSkConfig?.clientKey)
        assertEquals(config.amount, actualSkConfig?.amount)
        assertEquals(config.analyticsConfiguration, actualSkConfig?.analyticsConfiguration)
        assertEquals(config.isSubmitButtonVisible, actualSkConfig?.isSubmitButtonVisible)
    }
