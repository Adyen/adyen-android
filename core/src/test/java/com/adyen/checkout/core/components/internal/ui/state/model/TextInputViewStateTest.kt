...
internal class TextInputViewStateTest {
    ...
    @Test
    fun `when focus is requested for the field, then the view state asks for focus`() {
        // GIVEN
        ...
        // THEN
        assertTrue(viewState.isFocusRequested)
    }

    @Test
    fun `when focus is requested for another field, then this field does not ask for focus`() {
        // GIVEN
        val form = formOf(FIRST, LAST)
        ...
        // THEN
        assertFalse(viewState.isFocusRequested)
    }
