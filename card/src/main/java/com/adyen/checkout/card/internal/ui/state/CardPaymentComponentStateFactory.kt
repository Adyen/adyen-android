...
internal class CardPaymentComponentStateFactory(
    ...
    /**
     * We only prefill the card brand in the onSubmit payload if we are sure the card has a single brand identified on
     * the backend (reliable) or if the shopper manually selected that brand. In all other cases we cannot reliably
     * assume there is only one possible brand for the /payments call
     */
