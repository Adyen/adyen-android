...
/**
 * One row of the card form, carrying everything needed to render it. Render straight from this: a hidden field has no
 * element, and an element always has its data, so there is nothing to look up and nothing that can be missing.
 *
 * Keep new data on the element that needs it rather than beside the list. [CardNumberFormat] is on two elements for
 * that reason, and both derive it from the same brand state so they cannot disagree.
 */
