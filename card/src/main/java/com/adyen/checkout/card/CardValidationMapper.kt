...
class CardValidationMapper {
    fun mapCardNumberValidation(cardNumber: String, validation: CardNumberValidation): FieldState<String> {
        val fieldStateValidation = when (validation) {
            CardNumberValidation.INVALID_ILLEGAL_CHARACTERS -> Validation.Invalid(R.string.checkout_card_number_not_valid)
            ...
            CardNumberValidation.INVALID_UNSUPPORTED_BRAND -> Validation.Invalid(R.string.checkout_card_brand_not_supported, true)
