 ...
 *  - [toContentKey] is defined here instead of being imported from the recipe's own helpers.
 */
...
internal class SharedViewModelStoreNavEntryDecorator<T : Any>(
    ...
    companion object {
         ...
         *
         * @param key The [NavEntry.contentKey] of the parent, as given by [toContentKey].
         */
...
/**
 * The [NavEntry.contentKey] a [NavKey] is given when its entry does not declare one of its own.
 *
 * Mirrors nav3's own default so that a parent can be named by its key rather than by repeating that default.
 */
internal fun NavKey.toContentKey(): Any = toString()
