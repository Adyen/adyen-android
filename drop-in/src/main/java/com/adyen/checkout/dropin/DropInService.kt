...
abstract class DropInService : JobIntentService() {
    ...
    companion object {
        ...
        private const val PAYMENT_METHOD_EXTRA_KEY = "payment_method_extra"
        ...
        fun requestPaymentsCall(context: Context, paymentMethod: PaymentComponentData) {
            Logger.d(TAG, "requestPaymentsCall - ${paymentMethod.type}")
            ...
            workIntent.putExtra(PAYMENT_METHOD_EXTRA_KEY, paymentMethod)
    ...
    override fun onHandleWork(intent: Intent) {
        ...
        when (intent.getStringExtra(REQUEST_TYPE_KEY)) {
            PAYMENTS_REQUEST -> {
                val paymentMethodForRequest = intent.getParcelableExtra<PaymentComponentData>(PAYMENT_METHOD_EXTRA_KEY)
                askPaymentsCall(paymentMethodForRequest)
            }
            DETAILS_REQUEST -> {
    ...
    private fun askPaymentsCall(paymentMethod: PaymentComponentData) {
        Logger.d(TAG, "askPaymentsCall")

        // Merchant makes network call
        val paymentsCallResult = makePaymentsCall(PaymentComponentData.SERIALIZER.serialize(paymentMethod))
