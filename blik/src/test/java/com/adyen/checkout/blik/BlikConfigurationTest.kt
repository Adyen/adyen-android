...
internal class BlikConfigurationTest {
    ...
    fun `when creating the configuration through CheckoutConfiguration, then it should be the same as when the builder is used`() {
        ...
        val actual = checkoutConfiguration.getBlikConfiguration()

        val expected = BlikConfiguration.Builder(
            shopperLocale = Locale.US,
            environment = Environment.TEST,
            clientKey = TEST_CLIENT_KEY,
        )
    ...
    fun `when the configuration is mapped to CheckoutConfiguration, then CheckoutConfiguration is created correctly`() {
        val config = BlikConfiguration.Builder(
            shopperLocale = Locale.US,
            environment = Environment.TEST,
            clientKey = TEST_CLIENT_KEY,
        )
        ...
        val actualBlikConfig = actual.getBlikConfiguration()
        assertEquals(config.shopperLocale, actualBlikConfig?.shopperLocale)
        assertEquals(config.environment, actualBlikConfig?.environment)
        assertEquals(config.clientKey, actualBlikConfig?.clientKey)
        assertEquals(config.amount, actualBlikConfig?.amount)
        assertEquals(config.analyticsConfiguration, actualBlikConfig?.analyticsConfiguration)
        assertEquals(config.isSubmitButtonVisible, actualBlikConfig?.isSubmitButtonVisible)
    }
