...
internal class AuthenticationComponentTest(
    ...
    private fun AuthenticationComponent.handleAction(activity: Activity, uiCustomization: UiCustomization) {
        // Use reflection to call the private handleAction(Action, Activity, UiCustomization) method
        // since the public handleAction() triggers compose event flow which we can't use in unit tests
        val actionField = AuthenticationComponent::class.java.getDeclaredField("action")
        actionField.isAccessible = true
        val action = actionField.get(this) as Action

        val method = AuthenticationComponent::class.java.getDeclaredMethod(
            "handleAction",
            Action::class.java,
            Activity::class.java,
            UiCustomization::class.java,
        )
        method.isAccessible = true
        method.invoke(this, action, activity, uiCustomization)
    }
