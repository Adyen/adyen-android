...
fun AdyenLogo(
    ...
    contentDescription: String? = "Payment method logo",
    imageLoader: ImageLoader,
    @DrawableRes placeholder: Int = R.drawable.ic_placeholder_image,
    @DrawableRes errorFallback: Int = R.drawable.ic_placeholder_image,
) {
    // We can reuse the same logo path building logic.
    // NOTE: You might need to make these utility functions public if they are private.
    val densityExtension = LocalContext.current.resources.displayMetrics.densityDpi.getDensityExtension()
    val logoPath = buildLogoPath(size, txVariant, txSubVariant, densityExtension)
    val fullUrl = baseUrl + logoPath
