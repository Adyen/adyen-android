...
import com.adyen.checkout.threeds2.internal.data.model.FingerprintToken
import com.adyen.checkout.threeds2.internal.data.model.SubmitFingerprintResult
import com.adyen.checkout.threeds2.internal.data.model.ThreeDS2Serializer
import com.adyen.checkout.threeds2.internal.ui.model.ThreeDS2ComponentParams
...
internal class ThreeDS2Component(
    ...
    private fun handleAction(action: Action, activity: Activity) {
        if (action !is Threeds2Action) {
            emitError(
                GeneralError("Unsupported action")
            )
            return
        }
    ...
    private fun handleThreeds2ActionSubtype(
        ...
        if (token.isNullOrEmpty()) {
            ...
            emitError(
                GeneralError("3DS2 token not found.")
            )
            return
        }
    ...
    private fun handleThreeds2Action(
        ...
        if (action.subtype == null) {
            emitError(
                GeneralError("3DS2 Action subtype not found.")
            )
            return
        }
    ...
    internal fun identifyShopper(
        ...
        val configParameters = createAdyenConfigParameters(fingerprintToken) ?: run {
            ...
            emitError(
                GeneralError("Failed to create ConfigParameters.")
            )
            return
        }
        ...
        coroutineScope.launch(coroutineDispatcher + coroutineExceptionHandler) {
            ...
            if (authenticationRequestParameters == null) {
                ...
                emitError(
                    GeneralError("Failed to retrieve 3DS2 authentication parameters")
                )
                return@launch
            }
    ...
    internal fun challengeShopper(activity: Activity, encodedChallengeToken: String) {
        ...
        if (currentTransaction == null) {
            ...
            emitError(
                GeneralError("Failed to make challenge, missing reference to initial transaction.")
            )
            return
        }
