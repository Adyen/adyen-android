...
internal class MealVoucherConfigurationTest {
    ...
    fun `when creating the configuration through CheckoutConfiguration, then it should be the same as when the builder is used`() {
        ...
        val actual = checkoutConfiguration.getMealVoucherConfiguration()

        val expected = MealVoucherConfiguration.Builder(
            shopperLocale = Locale.US,
            environment = Environment.TEST,
            clientKey = TEST_CLIENT_KEY,
        )
    ...
    fun `when the configuration is mapped to CheckoutConfiguration, then CheckoutConfiguration is created correctly`() {
        val config = MealVoucherConfiguration.Builder(
            shopperLocale = Locale.US,
            environment = Environment.TEST,
            clientKey = TEST_CLIENT_KEY,
        )
        ...
        val actualMealVoucherConfig = actual.getMealVoucherConfiguration()
        assertEquals(config.shopperLocale, actualMealVoucherConfig?.shopperLocale)
        assertEquals(config.environment, actualMealVoucherConfig?.environment)
        assertEquals(config.clientKey, actualMealVoucherConfig?.clientKey)
        assertEquals(config.amount, actualMealVoucherConfig?.amount)
        assertEquals(config.analyticsConfiguration, actualMealVoucherConfig?.analyticsConfiguration)
        assertEquals(config.isSubmitButtonVisible, actualMealVoucherConfig?.isSubmitButtonVisible)
        assertEquals(config.isSecurityCodeRequired, actualMealVoucherConfig?.isSecurityCodeRequired)
    }
