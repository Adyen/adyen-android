...
internal class MealVoucherComponentTest(
    @Mock private val giftCardDelegate: GiftCardDelegate,
    @Mock private val genericActionDelegate: GenericActionDelegate,
    @Mock private val actionHandlingComponent: DefaultActionHandlingComponent,
    @Mock private val componentEventHandler: ComponentEventHandler<MealVoucherComponentState>,
) {
    private lateinit var component: MealVoucherComponent

    @BeforeEach
    fun before() {
        whenever(giftCardDelegate.viewFlow) doReturn MutableStateFlow(MealVoucherComponentViewType)
        whenever(genericActionDelegate.viewFlow) doReturn MutableStateFlow(null)

        component = MealVoucherComponent(
    ...
    fun `when observe is called then observe in delegates is called`() {
        ...
        val callback: (PaymentComponentEvent<MealVoucherComponentState>) -> Unit = {}
    ...
    fun `when component is initialized then view flow should match gift card delegate view flow`() = runTest {
        component.viewFlow.test {
            assert(awaitItem() is MealVoucherComponentViewType)
            expectNoEvents()
        }
    }
